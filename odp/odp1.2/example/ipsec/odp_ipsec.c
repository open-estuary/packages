...
typedef struct {
	...
	uint16_t tun_hdr_offset;            /**< Offset of tunnel header frombuffer start */
...
#if (1 == ODP_USE_HARDWARE_FOR_CRYPTO)
#define   MAX_DEV_NUM 16

char dev_name[MAX_DEV_NUM][32];
odp_acc_dev_handle gdev_handle;

odp_acc_dev_handle get_crypto_handle(void)
{
	char *pdev_name[16];
	uint32_t dev_num = 1;
	int   i = 0;
	...
	printf("\r\ncome at %dth line in %s()devname=%s,dev_num=%d\n", __LINE__, __func__, (char *)pdev_name[0],
	       dev_num);

	dev_handle = odp_acc_get_dev_handle((char *)pdev_name[0]);
...
void odp_crypto_compl(odp_crypto_session_t session, odp_crypto_op_result_t *result)
{
	printf("get in odp_crypto_compl  !");
}
#endif
...
void initialize_loop(char *intf)
{
	...
	printf("Created loop:%02i, queue mode (ATOMIC queues)\n"
	       "          default loop%02i-INPUT queue:%d\n"
	       "          default loop%02i-OUTPUT queue:%d\n"
...
void initialize_intf(char *intf)
{
	...
	printf("Created pktio:%02d, queue mode (ATOMIC queues)\n"
	       "          default pktio%02d-INPUT queue:%d\n"
	       "          source mac address %s\n",
...
#if (1 == ODP_USE_HARDWARE_FOR_CRYPTO)
	gdev_handle = get_crypto_handle();
	if (ODP_ACC_HANDLE_INVALID == gdev_handle) {
		EXAMPLE_ERR("Error: get_crypto_handle failed.\n");
