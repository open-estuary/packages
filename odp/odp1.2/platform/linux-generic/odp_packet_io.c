...
int odp_pktio_recv(odp_pktio_t id, odp_packet_t pkt_table[], unsigned int len)
{
...
int odp_pktio_mtu_set(odp_pktio_t id, uint16_t mtu)
{
	pktio_entry_t *entry;
	int ret;
	...
	if (entry->s.ops->mtu_set) {
		ret = entry->s.ops->mtu_set(entry, mtu);
	} else {
		ODP_DBG("mtu_set func absent!\n");
		ret = -1;
	}
