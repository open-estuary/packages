...
to:
	...
	pool->s.malloc_index++;
	pool->s.pool_malloc_addr += PACKET_HEAD_SIZE;
	if (pool->s.malloc_index >= pool->s.buf_num) {
		pool->s.pool_malloc_addr = pool->s.pool_mdata_addr;
...
unsigned long _packet_alloc_lock(pool_entry_t *pool)
{
	...
	odp_spinlock_lock((odp_spinlock_t *)&pool->s.blk_lock);
	vir_addr = _packet_alloc(pool);
	odp_spinlock_unlock((odp_spinlock_t *)&pool->s.blk_lock);
...
odp_packet_t odp_packet_alloc(odp_pool_t pool_hdl, uint32_t len)
{
	...
	return (odp_packet_t)(*pool->s.odp_pool_packet_alloc)(pool);
}

void _packet_free(packet_head_t *buf_head, pool_entry_t *pool)
{
	packet_head_t *free_addr = (packet_head_t *)pool->s.pool_free_addr;

	pool->s.free_index++;
	pool->s.pool_free_addr += PACKET_HEAD_SIZE;
	if (pool->s.free_index >= pool->s.buf_num) {
		pool->s.pool_free_addr = pool->s.pool_mdata_addr;
		pool->s.free_index = 0;
	}

	free_addr->index = buf_head->index;
	free_addr->phy_addr = buf_head->phy_addr;
	free_addr->vir_addr = buf_head->vir_addr;
	free_addr->packet_hdr = buf_head->packet_hdr;
}

void _packet_free_lock(packet_head_t *buf_head, pool_entry_t *pool)
{
	odp_spinlock_lock((odp_spinlock_t *)&pool->s.buf_lock);
	...
	odp_spinlock_unlock((odp_spinlock_t *)&pool->s.buf_lock);

	free_addr->index = buf_head->index;
	free_addr->phy_addr = buf_head->phy_addr;
	free_addr->pool_id  = buf_head->pool_id;
	free_addr->vir_addr = buf_head->vir_addr;
}

void odp_packet_free(odp_packet_t pkt)
{
	packet_head_t *buf_head = get_packet_head(pkt);
	pool_entry_t  *pool = get_pool_entry(buf_head->pool_id);

	(*pool->s.odp_pool_packet_free)(buf_head, pool);
}

int odp_packet_reset(odp_packet_t pkt, uint32_t len)
{
	odp_packet_hdr_t *const pkt_hdr = odp_packet_hdr(pkt);
	pool_entry_t *pool = get_pool_entry(get_packet_head(pkt)->pool_id);
...
{
	return (void *)odp_packet_hdr(pkt);
}

uint32_t odp_packet_buf_len(odp_packet_t pkt)
{
	return packet_get_len(pkt);
}
...
{
	return get_packet_head(pkt)->headroom;
}

uint32_t odp_packet_len(odp_packet_t pkt)
{
	return get_packet_head(pkt)->rx.size;
}

void packet_set_len(odp_packet_t pkt, uint32_t len)
{
	get_packet_head(pkt)->rx.size = len;
}

uint32_t packet_get_len(odp_packet_t pkt)
{
	return (uint32_t)get_packet_head(pkt)->rx.size;
}

uint32_t odp_packet_headroom(odp_packet_t pkt)
{
	return *GET_HEADROOM(pkt);
}

uint32_t odp_packet_tailroom(odp_packet_t pkt)
{
	return *GET_TAILROOM(pkt);
}
...
{
	uint32_t headroom = *GET_HEADROOM(pkt);
	uint32_t tailroom = *GET_TAILROOM(pkt);
	...
	get_packet_head(pkt)->rx.size += len;
	pkt -= len;
	SET_HEADROOM(pkt, headroom - len);
	SET_TAILROOM(pkt, tailroom);
...
void *odp_packet_pull_head(odp_packet_t pkt, uint32_t len)
{
	...
	if (len > get_packet_head(pkt)->rx.size)
		return NULL;
			...
			odp_packet_seg_t *seg)
{
	*seg = 0;
	...
	return odp_packet_push_head(pkt, offset);
}
...
{
	return pool_index_to_handle(get_packet_head(pkt)->pool_id);
}
...
{
	return odp_packet_alloc(pool_hdl, 2048);
}
