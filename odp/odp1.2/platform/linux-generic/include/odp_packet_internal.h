...
static inline packet_head_t *get_packet_head(odp_packet_t pkt)
{
	return (packet_head_t *)GET_PACKET_HEAD(pkt);
}
...
{
    	return (odp_packet_hdr_t *)get_packet_head(pkt)->packet_hdr;
}
			       ...
			       size_t		 size)
{
	...
	get_packet_head((odp_packet_t)pkt_hdr->buf_addr)->rx.size = size;
}
