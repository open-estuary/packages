		       ...
		       odp_queue_type_t type, odp_queue_param_t *param)
{
    ...
    case ODP_QUEUE_TYPE_ACCIN:
              queue->s.enqueue = accin_enqueue;
              queue->s.dequeue = accin_dequeue;
              queue->s.enqueue_multi = accin_enq_multi;
              queue->s.dequeue_multi = accin_deq_multi;
              break;    
	default:
