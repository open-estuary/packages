...
TAILQ_HEAD(, odp_uio_driver) odp_uio_drv_list;
...
void odp_uio_register(struct odp_uio_driver *driver)
{
	...
	if (!(driver->dev_init)) {
		PRINT("dev_init is null!\n");
		return;
	}

	if (!(driver->dev_uninit)) {
		PRINT("dev_uninit is null!\n");
		return;
	}

	/* 判断驱动名字是否已创建 */
	if (odp_check_uio_drv_is_reg(driver->name)) {
		PRINT("call odp_check_uio_drv_is_reg failed!\n");
		return;
	}
...
void odp_uio_unregister(struct odp_uio_driver *driver)
{
	...
	for (drv_temp = TAILQ_FIRST(&odp_uio_drv_list); drv_temp != NULL; drv_temp = drv_next) {
		if (strlen(drv_temp->name) == strlen(driver->name)) {
			len = strlen(driver->name);
			if (strncmp(drv_temp->name, driver->name, len) == 0) {
				TAILQ_REMOVE(&odp_uio_drv_list, drv_temp, next);
...
int odp_uio_drv_load_pro(void)
{
	DIR *d;
	struct dirent *file;
	char file_path_name[ODP_PATH_LEN + ODP_UIO_NAME_MAX_LEN]; /* 路径和名称 */
	void *dev_drv_dlopen_handle;

	if (!(d = opendir(ODP_DEV_DRV_PATH))) {
		PRINT("call opendir failed!\n");
		return (-1);
	}
	...
	while ((file = readdir(d)) != NULL) {
		...
		(void)strncpy(file_path_name, ODP_DEV_DRV_PATH, strlen(ODP_DEV_DRV_PATH) + 1);
		(void)strcat(file_path_name, file->d_name);

		dev_drv_dlopen_handle = dlopen(file_path_name, RTLD_LAZY);
		if (!dev_drv_dlopen_handle) {
			PRINT("call dlopen failed! dlopen err:%s.\n", dlerror());
			return (-1);
		}

		PRINT("open shared lib %s, addr = 0x%lx\n", file_path_name, *(unsigned long *)dev_drv_dlopen_handle);
	}
...
int odp_uio_dev_pro(void)
{
	...
	for (i = 0; i < max; i++) {
		...
		(void)strncpy(file_name, ODP_UIO_DEV_PATH, strlen(ODP_UIO_DEV_PATH) + 1);
		(void)strcat(file_name, file->d_name);

		dev = malloc(sizeof(struct odp_uio_device));

		if (!dev) {
			PRINT("call malloc failed!\n");
			closedir(d);
			return (-1);
		}

		memset(dev, 0, sizeof(struct odp_uio_device));
		(void)strncpy(dev->name, file->d_name, strlen(file->d_name) + 1);
		dev->numa_node = 0;
...
int odp_uio_mmap_pro(void)
{
	...
	TAILQ_FOREACH(dev, &odp_uio_dev_list, next)
	{
		...
		(void)strncpy(mmap_name, ODP_UIO_CLASS_PATH, strlen(ODP_UIO_CLASS_PATH) + 1);
		(void)strcat(mmap_name, dev->name);
		(void)strcat(mmap_name, "/maps/");

		uio_fd = open(file_name, O_RDWR);
		if (uio_fd < 0) {
			PRINT("call open %s failed!\n", file_name);
			return (-1);
		}

		if (!(d = opendir(mmap_name))) {
			PRINT("call opendir %s failed!\n", mmap_name);
			return (-1);
		}
		...
		while ((file = readdir(d)) != NULL) {
			...
			(void)strncpy(addr_name, mmap_name, strlen(mmap_name) + 1);
			(void)strncpy(size_name, mmap_name, strlen(mmap_name) + 1);
			...
			access_address = (void *)mmap(NULL, uio_size, PROT_READ | PROT_WRITE,
						      MAP_SHARED, uio_fd, UIO_OFFSET(i));
			if (access_address == (void *)-1) {
				PRINT("call mmap failed!\n");
...
int odp_uio_dev_drv_match(void)
{
	...
	TAILQ_FOREACH(dev, &odp_uio_dev_list, next)
	{
		...
		(void)strncpy(name, ODP_UIO_CLASS_PATH, strlen(ODP_UIO_CLASS_PATH) + 1);
		...
		{
			/* PRINT("drv->name = %s,  %s", drv->name, uio_drv_name); */
			if (strncmp(uio_drv_name, drv->name, strlen(drv->name)) == 0) {
				dev->driver = drv;
				flag = 1;
				break;
			}
...
int odp_uio_init(void)
{
	...
	TAILQ_INIT(&odp_uio_drv_list);
	TAILQ_INIT(&odp_uio_dev_list);
	TAILQ_INIT(&odp_uio_data_list);

	memset(dev_file, 0, sizeof(struct dev_file_info) * FILE_MAX);

	iret = odp_uio_drv_load_pro();
	if (iret) {
		PRINT("call odp_uio_drv_load_pro failed!\n");
		return iret;
	}

	iret = odp_uio_dev_pro();
	if (iret) {
		PRINT("call odp_uio_dev_pro failed!\n");
		return iret;
	}

	iret = odp_uio_mmap_pro();
	if (iret) {
		PRINT("call odp_uio_mmap_pro failed!\n");
		return iret;
	}

	iret = odp_uio_dev_drv_match();
	if (iret) {
		PRINT("call odp_uio_dev_drv_match failed!\n");
		return iret;
	}

	iret = odp_uio_dev_init();
	if (iret) {
		PRINT("call odp_uio_dev_init failed!\n");
		return iret;
	}
...
void *odp_uio_dev_data_alloc(char *name, uint32_t len)
{
	...
	if (odp_uio_check_name_is_alloc(name)) {
		PRINT("call odp_uio_check_name_is_alloc failed!\n");
		return NULL;
	}

	uio_data = (struct odp_uio_data *)malloc(len + sizeof(struct odp_uio_data));
	if (!uio_data) {
		PRINT("call malloc failed!\n");
		return NULL;
	}
	...
	uio_data->data = (void *)((uint8_t *)uio_data + sizeof(struct odp_uio_data));
...
void odp_uio_dev_data_free(char *name)
{
	...
	for (uio_data_temp = TAILQ_FIRST(&odp_uio_data_list); uio_data_temp != NULL; uio_data_temp = uio_date_next) {
		if (strlen(uio_data_temp->name) == strlen(name))
			if (strncmp(uio_data_temp->name, name, strlen(name)) == 0) {
				free((void *)((uint8_t *)uio_data_temp->data - sizeof(struct odp_uio_data)));
				TAILQ_REMOVE(&odp_uio_data_list, uio_data_temp, next);
				return;
			}
