...
echo "Try to pull Zuul docker image ..."
...
#Step 3: Try to pull zuul 5.0 docker image
if [ -z "$(docker images | grep 'openestuary/zuul' | grep -v grep)" ]  ; then
    docker pull openestuary/zuul:5.0
else 
    echo "zuul docker image has been pulled"
fi
