...
echo "Try to pull AliSQL Server docker image ..."
...
#Step 3: Try to pull latest alisql docker image
if [ -z "$(docker images | grep 'openestuary/alisql' | grep -v grep)" ]  ; then
    docker pull openestuary/alisql:5.6.22
else 
