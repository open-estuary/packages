...
#        handle  -lpthread -lm -lrt -lcrypt -ldl  and other strings inserted that might contain
...
$flags->{libs_r} =
  [@ldflags,@lib_r_opts,'',' -lpthread -lm -lrt -lcrypt -ldl ',''];
$flags->{embedded_libs} =
  [@ldflags,@lib_e_opts,'','',' -lpthread -lm -lrt -lcrypt -ldl ','',''];
