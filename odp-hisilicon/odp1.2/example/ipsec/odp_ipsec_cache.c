			     ...
			     odp_pool_t	       out_pool)
{
	...
	if (MAX_DB <= ipsec_cache->index) {
		EXAMPLE_ERR("Error:ipsec_cache->index.\n");
		return -1;
	}

	/* Verify SA mode match in case of cipher&auth */
	if (cipher_sa && auth_sa && (cipher_sa->mode != auth_sa->mode)) {
		EXAMPLE_ERR("Error:auth_sa->mode.\n");
		return -1;
	}
	...
	if (params.iv.length) {
		...
		if (ret != size) {
			EXAMPLE_ERR("Error:odp_random_data failed.\n");
			return -1;
		}
	}

#if (1 == ODP_USE_HARDWARE_FOR_CRYPTO)
	params.dev_handle = gdev_handle;
	params.odp_crypto_compl_pfn = odp_crypto_compl;
#endif

	/* Synchronous session create for now */
	if (odp_crypto_session_create(&params, &session, &ses_create_rc)) {
		EXAMPLE_ERR("Error:odp_crypto_session_create failed.\n");
		return -1;
	}

	if (ODP_CRYPTO_SES_CREATE_ERR_NONE != ses_create_rc) {
		EXAMPLE_ERR("Error:odp_crypto_session_create status failed.\n");
		return -1;
	}
					       ...
					       odph_esphdr_t *esp)
{
	...
	for (; NULL != entry; entry = entry->next) {
		if ((entry->src_ip != src_ip) || (entry->dst_ip != dst_ip))
			if ((entry->tun_src_ip != src_ip) || (entry->tun_dst_ip != dst_ip))
				continue;
