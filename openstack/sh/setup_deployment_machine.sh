...
client_hosts=(`cat ${filepath}/config/target_machine_hosts | awk '{print $2}'`)

# copy public keys to all machines
awk '{if ($0~/'"[0-9]+"'|localhost/)print $1}' ${filepath}/config/target_machine_hosts \
...
ansible all -u $USERNAME -K -i ${temp_hosts} -m copy -a "src=${filepath}/config/target_machine_hosts dest=~/"   2>/dev/null
ansible all -u $USERNAME -K -i ${temp_hosts} -m shell -a \
    "cat ~/target_machine_hosts >> /etc/hosts" --sudo

ansible all -u $USERNAME -K -i ${temp_hosts} -m copy -a "src=${filepath}/sh/setup_target_machines.sh dest=~/"   2>/dev/null
[ $? -ne 0 ] && die $LINENO "can not connect all the target machines"

ansible all -u $USERNAME -K -i ${temp_hosts} -m shell -a "/bin/bash ~/setup_target_machines.sh"
rm -fr ${temp_hosts}
